...
| **Play** | Two shapes of practice. **Design challenges**: freeform builds under locked weights/context, scored live against emergent floors. **Scenario games**: architect your way *out* — Crisis (fix a stack before the event lands), Campaign (survive three eras with the same codebase), Rescue (untangle the inherited mess) — where every change is billed in effort points from the migration ratings (drop-in 1 / migration 2 / rewrite 4) against a stage budget. Every game's solvability is re-verified by the audit on every catalog change. |
